...
//import NOOFQUOTE_FIELD from '@salesforce/schema/Opportunity.No_of_Quotes__c';
...
export default class OppProduct extends NavigationMixin(LightningElement) {
    label = {
        CheckPricing: labels.CheckPricing,
        Add: labels.Add,
        ...
        SelectGenericProduct: labels.SelectGenericProduct
    };
    ...
    get hasOLIFields() {
        return (this.oliObjInfos && this.oliFields && this.productFields)? true : false;
    }
    ...
    get isCheckPricingLocked() {
        return this.disabledCheckPricing || this.isOppLocked ? true : false;
    }
    ...
    getObjectInfo({ data, error }) {
        if (data) {
            this.oliObjInfos = data;
            this.oliFields = data.fields;
            console.log('oliObjInfos => ', data);
        } else if (error) {
            console.error('oliObjInfos ERROR => ', JSON.stringify(error)); // handle error properly
        }
    }
    ...
    getRecord({ data, error }) {
        ...
        if (data) {
            ...
            //this.noOfQuote = getFieldValue(data, NOOFQUOTE_FIELD);
    ...
    handleSearch(event) {
        ...
        if (searchTerm) {
            ...
            getProducts({ recordId: this.recordId, currencyIsoCode: this.currencyIsoCode, pricebookId: this.pricebookId, 
                searchTerm: searchTerm, salesOrgCode: this.salesOrgCode, languageCode: this.languageCode })
                .then(result => {
                    let results = [];
                    result.forEach(item => {
                        let result = {
