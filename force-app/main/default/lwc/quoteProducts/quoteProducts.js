...
export default class QuoteProducts extends NavigationMixin(LightningElement) {
    label = {
    ...
    handleItemChange(event) {
        ...
        if (value && (field === 'Quantity' || field === 'UnitPrice' )) {
    ...
    handleCheckPrice() {
        ...
        getSAPPricing({ recordId: this.recordId, quoteItems: items , headerDiscount: this.headerDiscount})
            .then(results => {
    ...
    @api
    handleCreateQuote() {
      console.log('c-quote-products', this.items);
      const selectedEvent = new CustomEvent('createquote', { detail: this.items });
